...
To facilitate maximum compatibility, Dendron builds on top of standard Markdown. Unfortunately, this means users will eventually run into conflicting behaviors if they use another tool that also builds on top of Markdown concurrently. 

It is often not immediately obvious why the conflict is happening, and require the users to use VSCode's [extension bisect](https://code.visualstudio.com/blogs/2021/02/16/extension-bisect) feature to detect which extension conflicts with Dendron.

Over the course of Dendron's existence, we have been making note of VSCode extensions that can cause conflicts when used with Dendron.
...
If you think the conflict listed here are resolved, or want them resolved so they could be used along with Dendron, please leave us an [issue](https://github.com/dendronhq/dendron/issues/new/choose) for a request of the feature that you want to see natively supported in Dendron, or a request to resolve the conflict with a particular extension.

Please note that these are wonderful extensions that have different takes on how to empower users with convenient tools to manipulate Markdown / notes and is only listed here for the ease of troubleshooting a conflict that happens while used with Dendron concurrently.
...
This extension has identical conflicts with the upstream version.
...
Foam and Dendron differs in how the notes are parsed and how metadata is attached to the note. This will cause problems for both extensions and there will be numerous features around manipulating Markdown that conflicts and overlaps.
...
This is a fork of [[Markdown Links|dendron://dendron.dendron-site/dendron.guides.troubleshooting.conflicting-extensions#markdown-links]] and has identical conflicts.
...
This is a forked version of [[Markdown Notes|dendron://dendron.dendron-site/dendron.guides.troubleshooting.conflicting-extensions#markdown-notes]] and has identical conflicts.