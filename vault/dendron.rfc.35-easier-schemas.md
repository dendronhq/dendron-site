...
This spec is focused on improving usability of Dendron's schemas, but not on extending schemas functionality beyond what is currently possible (that is covered in other RFC's, such as [[RFC 12 - Schema Improvements|dendron.rfc.12-schema-improvements.types]]). The proposals in this spec do not require modifications to Dendron's schema definition language.
